...
(function() {
  ...
  var rows = 8,
    ...
    colors = ["#d70f64", "#E97BAE", "#F4BDD6", "#54BAD1"],
    data = [
      {
        brand: "foodpanda",
        values: [12, 36, getRandomInt(100), getRandomInt(100)],
        variations: [getRandomInt(-100), getRandomInt(100), 0, 0]
      },
      {
        brand: "uber",
        values: [10, getRandomInt(100), getRandomInt(100), getRandomInt(100)],
        variations: [0, 0, 0, 0]
      },
      {
        brand: "mc donalds",
    ...
    for (var i = 1; i < n + 1; i++) {
      var thisValue = "bvalue" + i,
        ...
        line =
          ...
          i +
          '" class="text"/>' +
          ' Aided Awareness: <input type="text" name="' +
          thisValue +
          '1" id="' +
          thisValue +
          '1" class="input"/>' +
          ...
          thisVariation +
          '1" class="input"/>' +
          ' Consideration: <input type="text" name="' +
          thisValue +
          '2" id="' +
          thisValue +
          '2" class="input"/>' +
          ...
          thisVariation +
          '3" class="input"/>' +
          ' Preference: <input type="text" name="' +
          thisValue +
          '4" id="' +
          thisValue +
          '4" class="input"/>' +
  ...
  function fetchInputs() {
    switch (globalColors.value) {
      case "pink":
        ...
        break;
      case "red":
        colors = ["#D61F20", "#e67979", "#F2BBBC", "#83828A"];
        break;
      case "orange":
        colors = ["#FF6F00", "#FF9B4E", "#FFBD89", "#495092"];
    }
    for (var j = 1; j < rows + 1; j++) {
      for (var i = 1; i < 5; i++) {
        if (document.getElementById("bvalue" + j + i).value) {
          data[j - 1].values[i - 1] = Number(
            document.getElementById("bvalue" + j + i).value
          );
        }
        if (document.getElementById("variation" + j + i).value) {
          data[j - 1].variations[i - 1] = Number(
            document.getElementById("variation" + j + i).value
          );
        }
      }
    }
  ...
  function genLabels() {
    var nudge = 0;
    var width = 150;
    var texts = ["aided awareness", "consideration", "usage", "preference"];
    for (var i = 0; i < 4; i++) {
      var y = ystart;
      var x = xstart;
